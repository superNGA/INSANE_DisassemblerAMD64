...
// created : 13/01/2026
//
// purpose : Generate tables for VEX instructions using linux kernel x86 opcode map. 
//-------------------------------------------------------------------------
...
int main(int nArgs, char** args)
{
    ...
    // Dumping to file.
    DumpInstToFile(vecFinalTwoBytes,      "TwoByte.cpp",     "m_VEXTwoByteOpCodes");
    DumpInstToFile(vecFinalThreeBytes_38, "ThreeByte38.cpp", "m_VEXThreeByteOpCodes_38");
    DumpInstToFile(vecFinalThreeBytes_3A, "ThreeByte3A.cpp", "m_VEXThreeByteOpCodes_3A");
    ...
    for(size_t iLineIndex = 0llu; iLineIndex < vecLines.size(); iLineIndex++)
    {
        ...
        while(iInstEnd != INVALID_CHAR_POS)
        {
            ...
            if(bShouldStore == false)
                bShouldStore = szInst.find("(v)") != INVALID_CHAR_POS || szInst.find("(v1)") != INVALID_CHAR_POS;

            else if(szInst.find("(evo)") != INVALID_CHAR_POS || 
                    szInst.find("(ev)") != INVALID_CHAR_POS || 
                    szInst.find("(es)") != INVALID_CHAR_POS || 
                    szInst.find("(xop)") != INVALID_CHAR_POS)
                bShouldStore = false;
